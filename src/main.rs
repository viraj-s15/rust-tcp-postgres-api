...
fn handle_client(mut stream: TcpStream){
    ...
    match stream.read(&mut buffer) {
        Ok(size) => {
            request.push(String:from_utf8_lossy(&biffer[..size]).as_ref());

            let (status_line,content) = match &*request{
                r if r.starts_with("POST /users") => handle_post_request(r),
                r if r.starts_with("POST /users/") => handle_get_request(r),
                r if r.starts_with("POST /users") => handle_get_all_request(r),
        ...
        (Ok(id), Ok(mut client)) =>
            match client.query_one("SELECT * FROM Users WHERE id = $1", &[&id]) {
                Ok(row) => {
                    ...
                    (OK_RES.to_string(), serde_json::to_strin(&user).unwrap())
                }
                _ => (NOT_FOUND.to_string(), "User not found".to_string()),
            }
        
       _ => (INTERNAL_SERVER_ERROR.to_string(), "Error occured".to_string()),
    }
}
...
fn handle_delete_request(request: &str) -> (String, String) {
    match (get_id(&request).parse::<i32>(), Client::connect(DB_URL, NoTls)) {
        (Ok(id), Ok(mut client)) => {
            ...
            (OK_RES.to_string(), "User deleted".to_string())
        }
        _ => (INTERNAL_SERVER_ERROR.to_string(), "Error".to_string()),
    }
}
...
fn set_db() -> Result<(), PostgresError> {
    let mut client = Client::connet(DB_URL,NoTls)?;
...
fn get_ser_request_body(request: &str) -> Result<User, serde_json::Error> {
